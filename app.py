...
def process_video(video_url, api_key):
    ...
    try:
        ...
        update_progress("analyzing", "🤖 מנתחת עם Claude AI...", 50)
        summary_data = analyze_with_claude(transcript, api_key)

        # שלב 4: עריכה
        update_progress("editing", "✂️ עורכת את הסרטון...", 70)
        output_video = edit_video(video_path, summary_data)
...
2. **רגעים חשובים**: זהה בדיוק 5-6 רגעים מפתח שצריך להשאר בסרטון המקוצר
   עבור כל רגע, תן:
...
def edit_video(video_path, summary_data):
    """עריכת הסרטון"""
    try:
        ...
        if not moments:
            # אם אין רגעים, קח את התחלת הסרטון
            moments = [{"start_time": 0, "end_time": min(60, duration)}]

        # קיצוץ לפי רגעים
        clips = []
        for moment in moments:
            start = moment.get("start_time", 0) * 60  # דקות לשניות
            end = moment.get("end_time", 1) * 60
        ...
        if not clips:
            clips = [video.subclipped(0, min(60, duration))]

        # שלב את הקטעים
        final_video = concatenate_videoclips(clips)

        # שמור
        output_name = f"{summary_data.get('suggested_title', 'summarized')}.mp4"
